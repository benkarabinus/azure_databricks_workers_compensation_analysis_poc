...
Azure Databricks workspace plus a **dedicated ADLS Gen2 account** registered as a Unity
Catalog **external location** (the landing zone Session 1 ingests from).

**Output:** A running Premium workspace (Unity Catalog auto‑enabled, serverless available) and
...
- **Managed identity, not keys.** Unity Catalog accesses the storage account through an
  ...
  on an access connector — no account keys or secrets to rotate.- **Entra ID storage auth.** The storage account disables shared‑key auth
  (`shared_access_key_enabled = false`) and the `azurerm` provider manages the data plane with
  Microsoft Entra ID (`storage_use_azuread = true`), so it works on subscriptions whose Azure Policy
  forbids key‑based auth. Terraform grants the deploying identity **Storage Blob Data Contributor**
  (control‑plane Owner/Contributor doesn’t include blob data actions) so it can create the containers.
...
- **Managed storage, kept separate from landing.** Bronze/Silver/Gold are **managed tables**, so
  ...
  `MANAGED LOCATION` to a subpath of the `managed` external location, so all three schemas’
  managed tables physically land there. (For storage‑intensive production workloads, Databricks
  suggests separate **accounts** rather than just containers.)
...
- **Azure subscription** with permission to create resource groups, storage, a Databricks
  workspace, and **role assignments** — i.e. `Owner` or `User Access Administrator` on the target
...
Both providers authenticate with your Azure CLI session, so no Databricks personal access token
is needed. See [Databricks Terraform provider](https://learn.microsoft.com/azure/databricks/dev-tools/terraform/).
...
Variable reference: [Terraform input variables](https://developer.hashicorp.com/terraform/language/values/variables).
...
Docs: [terraform init](https://developer.hashicorp.com/terraform/cli/commands/init).
...
against that workspace (`azure_workspace_resource_id` in [terraform/providers.tf](terraform/providers.tf)),
and creating those Unity Catalog objects requires **account‑admin privileges** — so the first run is
done in **two phases with a one‑time admin step in between**.
...
(optionally preview with `terraform plan` first).

> **Auth:** the `databricks` provider is pinned to `auth_type = "azure-cli"`, so it uses your
> `az login` session and ignores any stale `~/.databrickscfg` profile or `DATABRICKS_*` env var (a
> leftover personal access token there otherwise causes `Invalid access token`).

Docs: [terraform apply](https://developer.hashicorp.com/terraform/cli/commands/apply) ·
[resource targeting](https://developer.hashicorp.com/terraform/cli/commands/plan#resource-targeting).

### 4. Verify

- **Azure portal:** confirm the resource group contains the storage account, workspace, and
  access connector.
...
Use `landing_path` as the value for the pipelines’ `landing_path` widget — it matches the default
...
Continue to **Session 1 — Foundations & Bronze Ingestion**, which creates the
`state_fund_poc` catalog and schemas and lands the six source files into Bronze.
