...
- **Return-to-Work (RTW)** — predict `days_to_rtw` (regression).
- **Fraud triage** — score claims with a `fraud_risk_score` to rank them for SIU review (classification). Framed as *investigation triage acceleration*, not automated detection.
...
| --- | --- | --- | --- |
| `claims_core.csv` | `bronze.raw_claims` | Core claim record (simulates a claims DB) — **the hub** | one row per claim |
| `hr_records.csv` | `bronze.raw_hr_records` | Employee tenure, job class, wage, employer (simulates an HR DB) | one row per worker |
| `medical_treatments.json` | `bronze.raw_medical_treatments` | Treatment events per claim (nested `events[]`, simulates a clinical feed) | one row per claim (nested) |
...
`claims` and `adjuster_notes` feed **both** use cases — that's the single-source-of-truth backbone.
...
- **Bronze** keeps the data exactly as it landed — including the dirty patterns (mixed date formats, casing/whitespace, invalid SSNs, duplicates, an orphan FK, a poison row). It adds only `_source_file` and `_ingested_at`.
- **Silver** cleans, conforms, deduplicates (latest ingest wins), parses dates to ISO, masks structured PII (SSN/DOB), redacts free-text PII, and structures notes with AI Functions. `silver.claims` is the hub; everything joins on `claim_id`.
...
`silver.claims` is the hub; every spoke joins on `claim_id` (or `employee_id` for the worker dimension). Gold tables are built by aggregating and feature-engineering these spokes back onto `claim_id`.
...
| --- | --- | --- |
| `raw_claims` | Claim hub, exactly as ingested | Contains the dirty patterns; `claimant_name`, `ssn`, `dob` still raw |
...
| `treatments` | One row per treatment event | PK `treatment_id`, FK `claim_id` | `events[]` exploded; `is_surgery` flag |
| `provider_billing` | One row per billing line | PK `billing_line_id`, FK `claim_id` | `billing_lines[]` exploded; amounts cast |
| `rtw_timeline` | Per-claim RTW milestones | PK `claim_id` | injury → first treatment → RTW dates |
| `adjuster_notes` | Structured + redacted notes | FK `claim_id` | PII redacted; `note_category` (`ai_classify`), `extracted_*` (`ai_extract`) |
...
Gold is the **business-ready** layer — purpose-built for consumption, not further transformation:

- **ML tables** (`rtw_features`, `fraud_features`) are flat, one-row-per-claim feature vectors with their labels, ready to hand directly to AutoML. Labels are intentionally partial — `days_to_rtw` exists only for **closed** claims, and `is_fraud` only for the **labeled SIU subset** — because that mirrors reality: you train on history, then score everything.
- **Serving** (`fraud_scores`) is the model's output, kept in Gold so the app and BI read a governed table rather than calling the model directly.
- **BI aggregate** (`rtw_outcomes_summary`) pre-summarizes outcomes for fast dashboards and natural-language questions.

Keeping these concerns in Gold means downstream consumers — analysts, Genie, the dashboard, the triage app, and the models — all read clean, governed, fit-for-purpose tables, with full lineage back to the original source files.
