...
session-0-setup/                       ← Environment readiness (Terraform: workspace + ADLS External Location)
  ...
  terraform/                           ← IaC: serverless-capable workspace + dedicated ADLS Gen2 as UC External Location
    versions.tf                        ← Terraform + provider constraints (azurerm, databricks, time)
    providers.tf                       ← azurerm (az login) + databricks (azure_workspace_resource_id) providers
    ...
    workspace.tf                       ← azurerm_databricks_workspace (premium; UC auto-enabled, serverless by default)
